...
import InfiniteFlag from '../components/InfiniteFlag.vue'
...
export default {
  ...
  methods: {
    start () {
      document.title = 'INY Media (Alpha)'
      this.loadTrand()
    },
    onStatus (status) {
    ...
    loadTrand () {
      Api.watchGetPopulars(this.page).then(({ data }) => {
        this.loader = 'data'
        if (data?.code === 200) {
          this.trandData = [...this.trandData, ...data?.popular]
