...
/* eslint-disable */
 ...
 export const Api = new Proxy({
   ...
   async userEditSave () {
    const jwt = localStorage.getItem(this.config.localStorageName.jwt)
    const res = await axios.get(API_PATH_METHOD + `user.editSave?v=1.0&jwt=${jwt}${(name ? '&name=' + name : '')}${(surname ? '&surname=' + surname : '')}${(birthday ? '&birthday=' + birthday : '')}${(gender ? '&gender=' + gender : '')}`)
    return res.data
   },
   isOnline (dateVisit) {
    const gap = 300000
