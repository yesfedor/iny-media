...
function CronUserFeedLoader (int $limit = 500, int $offset = 0) {
  // Get users list
  $query_user = "SELECT uid, telegram FROM User WHERE uid > :offset ORDER BY uid LIMIT $limit";
  $var_user = [
    ':offset' => $offset
  ];
  ...
  for ($i = 0; $i < count($user); $i++) {
    ...
    foreach ($timeByKinopoiskId as $itemKinopoiskId => $itemTime) {
      ...
      if (!$notifyCache['id']) {
        ...
        $global[$uid][] = CronUserFeedLoaderSender($uid, $telegram, $itemKinopoiskId, $feedData['binding'][$itemKinopoiskId]);
...
function CronUserFeedLoaderSender (int $uid, int $telegram, int $kinopoiskId, array $extendData) {
  return [
    $uid,
    $telegram,
    $kinopoiskId,
